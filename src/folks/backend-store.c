/* backend-store.c generated by valac 0.20.1.30-1b1d, the Vala compiler
...
static void ____lambda64_ (Block1Data* _data1_, GObject* o, GAsyncResult* r);
static void _____lambda64__gasync_ready_callback (GObject* source_object, GAsyncResult* res, gpointer self);
...
static void _____lambda64__gasync_ready_callback (GObject* source_object, GAsyncResult* res, gpointer self) {
	____lambda64_ (self, source_object, res);
...
static gboolean folks_backend_store_load_backends_co (FolksBackendStoreLoadBackendsData* _data_) {
	...
	folks_internal_profiling_point ("unloaded backends in BackendStore", NULL);
	_data_->_tmp12_ = NULL;
	...
	folks_internal_profiling_point ("found modules in BackendStore", NULL);
	{
	...
	folks_internal_profiling_point ("loaded modules in BackendStore", NULL);
	_data_->_data1_->backends_remaining = 1;
	{
		...
		while (TRUE) {
			...
			_folks_backend_store_backend_load_if_needed (_data_->self, _data_->_tmp78_, _____lambda64__gasync_ready_callback, block1_data_ref (_data_->_data1_));
		...
		if (!_data_->_tmp8_) {
			{
				...
				g_debug ("backend-store.vala:403: New backend '%s' prepared", _data_->_tmp12_);
			...
			__catch0_g_error:
			{
				...
				_data_->_tmp20_ = folks_backend_get_name (_data_->_tmp19_);
				_data_->_tmp21_ = _data_->_tmp20_;
				_data_->_tmp22_ = _data_->e;
				_data_->_tmp23_ = _data_->_tmp22_->message;
				g_warning ("backend-store.vala:409: Error preparing Backend '%s': %s", _data_->_tmp21_, _data_->_tmp23_);
	...
	if (!_data_->_tmp3_) {
		...
		if (_data_->_tmp9_ != NULL) {
			...
			__catch1_g_error:
			{
				...
				g_warning ("backend-store.vala:433: Error unpreparing Backend '%s': %s", _data_->_tmp13_, _data_->_tmp15_);
				_g_error_free0 (_data_->e);
			}
			__finally1:
			if (_data_->_inner_error_ != NULL) {
...
void folks_backend_store_add_backend (FolksBackendStore* self, FolksBackend* backend) {
	...
	if (_tmp9_) {
		...
		folks_backend_unprepare (G_TYPE_CHECK_INSTANCE_CAST (_tmp10_, FOLKS_TYPE_BACKEND, FolksBackend), NULL, NULL);
		_tmp11_ = self->priv->_prepared_backends;
		_tmp12_ = backend_existing;
		_tmp13_ = folks_backend_get_name (G_TYPE_CHECK_INSTANCE_CAST (_tmp12_, FOLKS_TYPE_BACKEND, FolksBackend));
		_tmp14_ = _tmp13_;
...
static gboolean _folks_backend_store_backend_is_enabled (FolksBackendStore* self, const gchar* name) {
	...
	if (_tmp7_ != NULL) {
		...
		_tmp8_ = self->priv->_backends_disabled;
		_tmp9_ = name;
		_tmp10_ = gee_abstract_collection_contains ((GeeAbstractCollection*) G_TYPE_CHECK_INSTANCE_CAST (_tmp8_, FOLKS_TYPE_SMALL_SET, FolksSmallSet), _tmp9_);
		_tmp6_ = _tmp10_;
	} else {
		_tmp6_ = FALSE;
	}
	...
	}
	{
		...
		_tmp13_ = self->priv->_backends_key_file;
		_tmp14_ = folks_backend_store_KEY_FILE_GROUP_ALL_OTHERS;
		_tmp15_ = g_key_file_get_boolean (_tmp13_, _tmp14_, "enabled", &_inner_error_);
		_tmp12_ = _tmp15_;
	...
	__catch2_g_key_file_error:
	{
		...
		_inner_error_ = NULL;
		_tmp17_ = e;
		if (!g_error_matches (_tmp17_, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_GROUP_NOT_FOUND)) {
			GError* _tmp18_;
			_tmp18_ = e;
			_tmp16_ = !g_error_matches (_tmp18_, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND);
		} else {
			_tmp16_ = FALSE;
		}
		_tmp19_ = _tmp16_;
		if (_tmp19_) {
			...
			_tmp22_ = _tmp20_;
			g_warning ("Couldn't determine whether to enable or disable " "backends not listed in backend key file. Defaulting to %s.", _tmp22_);
		} else {
			const gchar* _tmp23_ = NULL;
			gboolean _tmp24_;
			const gchar* _tmp25_;
			_tmp24_ = all_others_enabled;
			if (_tmp24_) {
				_tmp23_ = "Enabling";
			} else {
				_tmp23_ = "Disabling";
			...
			_tmp25_ = _tmp23_;
			g_debug ("No catch-all entry in the backend key file. %s " "unlisted backends.", _tmp25_);
		}
		_g_error_free0 (e);
	}
	__finally2:
	...
	enabled = TRUE;
	{
		...
		_tmp27_ = self->priv->_backends_key_file;
		_tmp28_ = name;
		_tmp29_ = g_key_file_get_boolean (_tmp27_, _tmp28_, "enabled", &_inner_error_);
		_tmp26_ = _tmp29_;
		if (_inner_error_ != NULL) {
			if (_inner_error_->domain == G_KEY_FILE_ERROR) {
	...
	__catch3_g_key_file_error:
	{
		...
		if (g_error_matches (_tmp31_, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_GROUP_NOT_FOUND)) {
			...
			GError* _tmp32_;
			_tmp32_ = e;
			_tmp30_ = g_error_matches (_tmp32_, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND);
		}
		_tmp33_ = _tmp30_;
		if (_tmp33_) {
			...
			_tmp36_ = name;
			_tmp37_ = _tmp34_;
			_tmp38_ = folks_backend_store_KEY_FILE_GROUP_ALL_OTHERS;
			g_debug ("Found no entry for backend '%s'.enabled in backend " "keyfile. %s according to '%s' setting.", _tmp36_, _tmp37_, _tmp38_);
			_tmp39_ = all_others_enabled;
			enabled = _tmp39_;
		} else {
			...
			gboolean _tmp43_;
			_tmp41_ = e;
			if (!g_error_matches (_tmp41_, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_GROUP_NOT_FOUND)) {
				GError* _tmp42_;
				_tmp42_ = e;
				_tmp40_ = !g_error_matches (_tmp42_, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND);
			} else {
				_tmp40_ = FALSE;
			}
			_tmp43_ = _tmp40_;
...
static gboolean _folks_backend_store_get_modules_from_dir_co (FolksBackendStoreGetModulesFromDirData* _data_) {
	...
	_data_->_tmp2_ = _data_->_tmp1_;
	g_debug ("backend-store.vala:605: Searching for modules in folder '%s' ..", _data_->_tmp2_);
	...
	_data_->attributes = _data_->_tmp3_;
	{
		...
		_data_->_state_ = 2;
		g_file_enumerator_next_files_async (_data_->_tmp8_, _data_->_tmp9_, G_PRIORITY_DEFAULT, NULL, _folks_backend_store_get_modules_from_dir_ready, _data_);
		return FALSE;
		_state_2:
		_data_->_tmp10_ = NULL;
		...
		for (_data_->info_it = _data_->info_collection; _data_->info_it != NULL; _data_->info_it = _data_->info_it->next) {
			...
			_data_->info = _data_->_tmp21_;
			{
				...
				if (_data_->_tmp34_ == G_FILE_TYPE_DIRECTORY) {
					...
					if (_data_->_tmp37_ != NULL) {
						{
							...
							while (TRUE) {
								...
								_data_->_tmp47_ = gee_iterator_get (_data_->_tmp46_);
								_data_->entry = (GeeMapEntry*) _data_->_tmp47_;
								_data_->_tmp48_ = _data_->modules_final;
								_data_->_tmp49_ = _data_->entry;
								_data_->_tmp50_ = gee_map_entry_get_key (_data_->_tmp49_);
								_data_->_tmp51_ = _data_->_tmp50_;
								_data_->_tmp52_ = _data_->entry;
					...
					_data_->_tmp56_ = _data_->mime;
					if (g_strcmp0 (_data_->_tmp56_, "application/x-sharedlib") == 0) {
						_data_->_tmp57_ = _data_->is_symlink;
						_data_->_tmp55_ = !_data_->_tmp57_;
					} else {
						_data_->_tmp55_ = FALSE;
					}
					_data_->_tmp58_ = _data_->_tmp55_;
					if (_data_->_tmp58_) {
						...
						if (_data_->_tmp61_ != NULL) {
							...
							_data_->_tmp64_ = _data_->file;
							gee_abstract_map_set ((GeeAbstractMap*) _data_->_tmp62_, (const gchar*) _data_->_tmp63_, _data_->_tmp64_);
						}
						_g_free0 (_data_->path);
					} else {
						_data_->_tmp65_ = _data_->mime;
						if (_data_->_tmp65_ == NULL) {
							_data_->_tmp66_ = _data_->file;
							_data_->_tmp67_ = NULL;
							_data_->_tmp67_ = g_file_get_path (_data_->_tmp66_);
							_data_->_tmp68_ = _data_->_tmp67_;
							g_warning ("backend-store.vala:668: The content type of '%s' could not be determin" \
"ed. Have you installed shared-mime-info?", _data_->_tmp68_);
							_g_free0 (_data_->_tmp68_);
						} else {
							_data_->_tmp72_ = _data_->_path;
							if (_data_->_tmp72_ == NULL) {
								_data_->_tmp73_ = _data_->mime;
								_data_->_tmp71_ = g_strcmp0 (_data_->_tmp73_, "application/x-sharedlib") != 0;
							} else {
								_data_->_tmp71_ = FALSE;
							}
							_data_->_tmp74_ = _data_->_tmp71_;
							if (_data_->_tmp74_) {
								_data_->_tmp75_ = _data_->mime;
								_data_->_tmp70_ = g_strcmp0 (_data_->_tmp75_, "application/x-shared-library-la") != 0;
							} else {
								_data_->_tmp70_ = FALSE;
							}
							_data_->_tmp76_ = _data_->_tmp70_;
							if (_data_->_tmp76_) {
								_data_->_tmp77_ = _data_->mime;
								_data_->_tmp69_ = g_strcmp0 (_data_->_tmp77_, "inode/directory") != 0;
							} else {
								_data_->_tmp69_ = FALSE;
							}
							_data_->_tmp78_ = _data_->_tmp69_;
							if (_data_->_tmp78_) {
								_data_->_tmp79_ = _data_->file;
								_data_->_tmp80_ = NULL;
								_data_->_tmp80_ = g_file_get_path (_data_->_tmp79_);
								_data_->_tmp81_ = _data_->_tmp80_;
								_data_->_tmp82_ = _data_->mime;
								g_warning ("backend-store.vala:681: The content type of '%s' appears to be '%s' wh" \
"ich looks suspicious. Have you installed shared-mime-info?", _data_->_tmp81_, _data_->_tmp82_);
	...
	_data_->_tmp84_ = NULL;
	_data_->_tmp84_ = g_file_get_path (_data_->_tmp83_);
	_data_->_tmp85_ = _data_->_tmp84_;
	g_debug ("backend-store.vala:685: Finished searching for modules in folder '%s'", _data_->_tmp85_);
...
static void _folks_backend_store_load_module_from_file (FolksBackendStore* self, GFile* file) {
	...
	if (_tmp10_ == NULL) {
		...
		g_warning ("backend-store.vala:706: Failed to load module from path '%s': %s", _tmp11_, _tmp12_);
	...
	if (!_tmp16_) {
		...
		g_warning ("backend-store.vala:720: Failed to find entry point function '%s' in '%" \
"s': %s", "module_init", _tmp17_, _tmp18_);
	...
	g_debug ("backend-store.vala:738: Loaded module source: '%s'", _tmp27_);
	_g_module_close0 (_module);
	_g_free0 (file_path);
	_g_free0 (_file_path);
}
...
static gboolean _folks_backend_store_load_disabled_backend_names_co (FolksBackendStoreLoadDisabledBackendNamesData* _data_) {
	...
	if (_data_->_tmp2_ != NULL) {
		...
		if (_data_->_tmp12_ != NULL) {
			g_debug ("backend-store.vala:813: Backends limited by FOLKS_BACKENDS_ALLOWED:");
			...
			_data_->_tmp13__length1 = _data_->tokens_length1;
			{
				...
				for (_data_->_vala1_s_it = 0; _data_->_vala1_s_it < _data_->_tmp13__length1; _data_->_vala1_s_it = _data_->_vala1_s_it + 1) {
					_data_->_vala1_s = _data_->_vala1_s_collection[_data_->_vala1_s_it];
					{
						...
						g_debug ("backend-store.vala:816: Backend '%s' is allowed", _data_->_tmp14_);
					}
				}
			}
			g_debug ("backend-store.vala:818: All other backends disabled by FOLKS_BACKENDS_" \
"ALLOWED");
		}
			...
			for (_data_->_vala2_s_it = 0; _data_->_vala2_s_it < _data_->_tmp22__length1; _data_->_vala2_s_it = _data_->_vala2_s_it + 1) {
				_data_->_vala2_s = _data_->_vala2_s_collection[_data_->_vala2_s_it];
				{
					...
					if (g_strcmp0 (_data_->_tmp23_, "") != 0) {
						...
						g_debug ("backend-store.vala:836: Backend '%s' disabled by FOLKS_BACKENDS_DISABL" \
"ED", _data_->_tmp24_);
	...
	__catch6_g_error:
	{
		...
		if (!g_error_matches (_data_->_tmp56_, G_IO_ERROR, G_IO_ERROR_NOT_FOUND)) {
			...
			g_warning ("backend-store.vala:884: The backends key file '%s' could not be loaded" \
...
static gboolean _folks_backend_store_save_key_file_co (FolksBackendStoreSaveKeyFileData* _data_) {
	...
	_data_->_tmp4_ = _data_->_tmp3_;
	g_debug ("backend-store.vala:902: Saving backend key file '%s'.", _data_->_tmp4_);
	...
	__catch7_g_error:
	{
		...
		g_warning ("backend-store.vala:919: Could not write updated backend key file '%s':" \
...
static GObject * folks_backend_store_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	_tmp10_ = gee_hash_map_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, G_TYPE_POINTER, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	_g_object_unref0 (self->priv->_modules);
	self->priv->_modules = _tmp10_;
	...
	self->priv->_backend_hash = _tmp11_;
	_tmp12_ = gee_hash_map_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, FOLKS_TYPE_BACKEND, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	_g_object_unref0 (self->priv->_prepared_backends);
	self->priv->_prepared_backends = _tmp12_;
	_tmp13_ = self->priv->_prepared_backends;
	_tmp14_ = gee_abstract_map_get_read_only_view ((GeeAbstractMap*) _tmp13_);
	_tmp15_ = _tmp14_;
	_g_object_unref0 (self->priv->_prepared_backends_ro);
	self->priv->_prepared_backends_ro = _tmp15_;
