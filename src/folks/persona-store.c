/* persona-store.c generated by valac 0.20.1.30-1b1d, the Vala compiler
...
const gchar* folks_persona_store_detail_key (FolksPersonaDetail detail) {
	...
	_tmp4_ = detail;
	_tmp5_ = FOLKS_PERSONA_STORE__PERSONA_DETAIL[_tmp4_];
	result = _tmp5_;
	return result;
}
...
void _folks_persona_store_emit_personas_changed (FolksPersonaStore* self, GeeSet* added, GeeSet* removed, const gchar* message, FolksPersona* actor, FolksGroupDetailsChangeReason reason) {
	...
	FolksGroupDetailsChangeReason _tmp39_;
	...
	if (_tmp17_) {
		...
		if (_tmp18_ == NULL) {
			...
			if (_tmp20_ == NULL) {
				GeeHashSet* _tmp21_;
				_tmp21_ = gee_hash_set_new (FOLKS_TYPE_PERSONA, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL, NULL, NULL, NULL);
				_g_object_unref0 (_removed);
	...
	_tmp28_ = _tmp27_;
	folks_internal_profiling_point ("emitting PersonaStore::personas-changed " "(ID: %s, count: %u)", _tmp22_, _tmp25_ + _tmp28_, NULL);
	_tmp29_ = _added;
	_tmp30_ = gee_set_get_read_only_view (G_TYPE_CHECK_INSTANCE_CAST (_tmp29_, GEE_TYPE_SET, GeeSet));
	_tmp31_ = _tmp30_;
	_tmp32_ = _tmp31_;
	_tmp33_ = _removed;
	_tmp34_ = gee_set_get_read_only_view (G_TYPE_CHECK_INSTANCE_CAST (_tmp33_, GEE_TYPE_SET, GeeSet));
	_tmp35_ = _tmp34_;
	_tmp36_ = _tmp35_;
	_tmp37_ = message;
	_tmp38_ = actor;
	_tmp39_ = reason;
	g_signal_emit_by_name (self, "personas-changed", _tmp32_, _tmp36_, _tmp37_, _tmp38_, _tmp39_);
...
void folks_persona_store_set_trust_level (FolksPersonaStore* self, FolksPersonaStoreTrust value) {
	...
	if (_tmp0_ > _tmp2_) {
		...
		g_debug ("persona-store.vala:625: Unable to lower Persona Store trust_level");
	}
	g_object_notify ((GObject *) self, "trust-level");
}
...
static GObject * folks_persona_store_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	_tmp0_ = self->priv->_id;
	g_debug ("persona-store.vala:327: Constructing PersonaStore ‘%s’ (%p)", _tmp0_, self);
	return obj;
}
...
static void folks_persona_store_finalize (GObject* obj) {
	...
	self = G_TYPE_CHECK_INSTANCE_CAST (obj, FOLKS_TYPE_PERSONA_STORE, FolksPersonaStore);
	_tmp0_ = self->priv->_id;
	g_debug ("persona-store.vala:332: Destroying PersonaStore ‘%s’ (%p)", _tmp0_, self);
	_g_free0 (self->priv->_display_name);
	_g_free0 (self->priv->_id);
