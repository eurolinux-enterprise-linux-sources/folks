/* import.c generated by valac 0.20.1.30-1b1d, the Vala compiler
...
gint folks_import_tool_main (gchar** args, int args_length1) {
	...
	if (_tmp15_) {
		gchar* _tmp16_;
		_tmp16_ = g_strdup (FOLKS_IMPORT_TOOL_DEFAULT_SOURCE);
		_g_free0 (folks_import_tool_source);
		folks_import_tool_source = _tmp16_;
	}
	_tmp17_ = folks_individual_aggregator_new ();
	aggregator = _tmp17_;
	_g_object_unref0 (aggregator);
	aggregator = NULL;
	_tmp18_ = g_main_loop_new (NULL, FALSE);
	_data1_->main_loop = _tmp18_;
	_data1_->success = FALSE;
	folks_import_tool_import (___lambda2__gasync_ready_callback, block1_data_ref (_data1_));
	_tmp19_ = _data1_->main_loop;
	g_main_loop_run (_tmp19_);
	_tmp21_ = _data1_->success;
	if (_tmp21_) {
		_tmp20_ = 0;
	} else {
		_tmp20_ = 1;
