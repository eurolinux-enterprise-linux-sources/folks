/* command-personas.c generated by valac 0.34.0.3-63fa6, the Vala compiler
...
static void folks_inspect_commands_personas_real_run (FolksInspectCommand* base, const gchar* command_string, GAsyncReadyCallback _callback_, gpointer _user_data_) {
	FolksInspectCommandsPersonas * self;
	FolksInspectCommandsPersonasRunData* _data_;
	FolksInspectCommandsPersonas* _tmp0_ = NULL;
...
static gint folks_inspect_commands_personas_real_run_finish (FolksInspectCommand* base, GAsyncResult* _res_) {
	gint result;
	FolksInspectCommandsPersonasRunData* _data_;
...
static gboolean folks_inspect_commands_personas_real_run_co (FolksInspectCommandsPersonasRunData* _data_) {
		...
		while (TRUE) {
			...
			_data_->individual = (FolksIndividual*) _data_->_tmp13_;
			{
				...
				while (TRUE) {
					...
					folks_inspect_utils_print_persona (_data_->_tmp28_);
					_data_->_tmp29_ = NULL;
					_data_->_tmp29_ = _data_->command_string;
	...
	_data_->_tmp31_ = _data_->found_persona;
	if (!_data_->_tmp31_) {
		_data_->_tmp32_ = NULL;
		_data_->_tmp32_ = _data_->command_string;
		_data_->_tmp30_ = _data_->_tmp32_ != NULL;
	} else {
		_data_->_tmp30_ = FALSE;
	}
	if (_data_->_tmp30_) {
		_data_->_tmp33_ = NULL;
		_data_->_tmp33_ = _data_->command_string;
		folks_inspect_utils_print_line ("Unrecognised persona UID '%s'.", _data_->_tmp33_);
		_data_->result = 1;
		if (_data_->_state_ == 0) {
...
static gchar** folks_inspect_commands_personas_real_complete_subcommand (FolksInspectCommand* base, const gchar* subcommand, int* result_length1) {
	...
	_tmp3_ = (_tmp2_ != NULL) ? _vala_array_dup10 (_tmp2_, _vala_array_length (_tmp1_)) : ((gpointer) _tmp2_);
...
static void folks_inspect_commands_personas_class_init (FolksInspectCommandsPersonasClass * klass) {
	...
	((FolksInspectCommandClass *) klass)->run = folks_inspect_commands_personas_real_run;
	((FolksInspectCommandClass *) klass)->run_finish = folks_inspect_commands_personas_real_run_finish;
	((FolksInspectCommandClass *) klass)->complete_subcommand = folks_inspect_commands_personas_real_complete_subcommand;
