/* ofono-persona-store.c generated by valac 0.20.1.30-1b1d, the Vala compiler
...
static gchar** _folks_backends_ofono_persona_store_split_all_vcards (FolksBackendsOfonoPersonaStore* self, const gchar* all_vcards, int* result_length1) {
	...
	_tmp5__length1 = lines_length1;
	{
		...
		for (line_it = 0; line_it < _tmp5__length1; line_it = line_it + 1) {
			...
			line = _tmp6_;
			{
				...
				_tmp14_ = _tmp13_;
				_tmp15_ = g_strcmp0 (_tmp14_, "END:VCARD") == 0;
				_g_free0 (_tmp14_);
				if (_tmp15_) {
					gchar** _tmp16_;
					...
					gchar* _tmp19_;
					_tmp16_ = vcards;
					_tmp16__length1 = vcards_length1;
					_tmp17_ = vcard;
					_tmp18_ = g_strdup (_tmp17_);
					_vala_array_add2 (&vcards, &vcards_length1, &_vcards_size_, _tmp18_);
					_tmp19_ = g_strdup ("");
					_g_free0 (vcard);
					vcard = _tmp19_;
				}
	...
	_tmp20_ = vcards;
	_tmp20__length1 = vcards_length1;
	if (result_length1) {
...
static void __folks_backends_ofono_persona_store_property_changed_org_ofono_sim_manager_property_changed (orgofonoSimManager* _sender, const gchar* property, GVariant* value, gpointer self) {
	_folks_backends_ofono_persona_store_property_changed (self, property, value);
}
...
static gboolean folks_backends_ofono_persona_store_real_prepare_co (FolksBackendsOfonoPersonaStorePrepareData* _data_) {
	...
	}
	{
		...
		_data_->_state_ = 1;
		g_async_initable_new_async (ORG_OFONO_TYPE_PHONEBOOK_PROXY, 0, NULL, folks_backends_ofono_persona_store_prepare_ready, _data_, "g-flags", 0, "g-name", "org.ofono", "g-bus-type", G_BUS_TYPE_SYSTEM, "g-object-path", (const gchar*) _data_->_tmp7_, "g-interface-name", "org.ofono.Phonebook", NULL);
		return FALSE;
		_state_1:
		_data_->_tmp8_ = NULL;
		_data_->_tmp8_ = g_async_initable_new_finish (_data_->_source_object_, _data_->_res_, &_data_->_inner_error_);
		_data_->_tmp6_ = (orgofonoPhonebook*) _data_->_tmp8_;
	...
	__catch2_g_dbus_error:
	{
		...
		g_warning ("ofono-persona-store.vala:231: DBus Error has occurred when fetching of" \
"ono phonebook, %s", _data_->_tmp34_);
	...
	__catch2_g_io_error:
	{
		...
		g_warning ("ofono-persona-store.vala:236: IO Error has occurred when fetching ofon" \
