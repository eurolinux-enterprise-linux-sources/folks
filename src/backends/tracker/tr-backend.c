/* tr-backend.c generated by valac 0.20.1.30-1b1d, the Vala compiler
	...
	}
	{
		_data_->self->priv->_prepare_pending = TRUE;
		{
			...
			while (TRUE) {
				...
				g_signal_emit_by_name ((FolksBackend*) _data_->self, "persona-store-removed", _data_->_tmp14_);
...
static void folks_backends_tr_backend_class_init (FolksBackendsTrBackendClass * klass) {
	...
	FOLKS_BACKEND_CLASS (klass)->enable_persona_store = folks_backends_tr_backend_real_enable_persona_store;
	FOLKS_BACKEND_CLASS (klass)->disable_persona_store = folks_backends_tr_backend_real_disable_persona_store;
	FOLKS_BACKEND_CLASS (klass)->set_persona_stores = folks_backends_tr_backend_real_set_persona_stores;
