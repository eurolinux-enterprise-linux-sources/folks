/* trf-persona-store.c generated by valac 0.35.3.10-6b27, the Vala compiler
...
struct _TrfPersonaStoreAddPersonaFromDetailsData {
	...
	gboolean _tmp375_;
	TrfPersona* _tmp376_;
	GLoadableIcon* _tmp377_;
	TrfPersona* _tmp378_;
	GLoadableIcon* _tmp379_;
...
static gboolean trf_persona_store_real_add_persona_from_details_co (TrfPersonaStoreAddPersonaFromDetailsData* _data_) {
																	...
																	if (g_strcmp0 (_data_->_tmp288_, _data_->_tmp289_) == 0) {
																		...
																		if (g_strcmp0 (_data_->_tmp315_, _data_->_tmp316_) == 0) {
																			...
																			if (g_strcmp0 (_data_->_tmp333_, _data_->_tmp334_) == 0) {
																				...
																				_data_->_tmp351_ = _data_->k;
																				_data_->_tmp352_ = folks_persona_store_get_id ((FolksPersonaStore*) _data_->self);
																				_data_->_tmp353_ = _data_->_tmp352_;
																				_data_->_tmp354_ = g_error_new (FOLKS_PERSONA_STORE_ERROR, FOLKS_PERSONA_STORE_ERROR_INVALID_ARGUMENT, _ ("Unrecognized parameter ‘%s’ passed to persona store ‘%s’."), _data_->_tmp351_, _data_->_tmp353_);
																				_data_->_inner_error_ = _data_->_tmp354_;
	...
	_data_->_tmp355_ = _data_->builder;
	tracker_sparql_builder_insert_close (_data_->_tmp355_);
	_data_->ret = NULL;
	_data_->_tmp356_ = _data_->builder;
	_data_->_tmp357_ = tracker_sparql_builder_get_result (_data_->_tmp356_);
	_data_->_tmp358_ = _data_->_tmp357_;
	_data_->_state_ = 3;
	_trf_persona_store_insert_persona (_data_->self, _data_->_tmp358_, "p", trf_persona_store_add_persona_from_details_ready, _data_);
	return FALSE;
	_state_3:
	_data_->_tmp359_ = _trf_persona_store_insert_persona_finish (_data_->self, _data_->_res_, &_data_->_inner_error_);
	_data_->contact_urn = _data_->_tmp359_;
	...
	_data_->_tmp360_ = _data_->contact_urn;
	if (_data_->_tmp360_ != NULL) {
		_data_->_tmp361_ = _data_->contact_urn;
		_data_->_tmp362_ = g_strdup_printf (" FILTER(?_contact = <%s>) ", _data_->_tmp361_);
		_data_->filter = _data_->_tmp362_;
		_data_->_tmp363_ = _data_->filter;
		_data_->_tmp364_ = g_strdup_printf (TRF_PERSONA_STORE__INITIAL_QUERY, _data_->_tmp363_);
		_data_->query = _data_->_tmp364_;
		_data_->_tmp365_ = _data_->query;
		_data_->_state_ = 4;
		_trf_persona_store_do_add_contacts (_data_->self, _data_->_tmp365_, trf_persona_store_add_persona_from_details_ready, _data_);
		return FALSE;
		_state_4:
		_data_->_tmp366_ = _trf_persona_store_do_add_contacts_finish (_data_->self, _data_->_res_);
		_data_->ret_personas = _data_->_tmp366_;
		{
			_data_->_tmp367_ = _data_->ret_personas;
			_data_->_tmp368_ = gee_abstract_collection_iterator ((GeeAbstractCollection*) _data_->_tmp367_);
			_data_->_p_it = _data_->_tmp368_;
			while (TRUE) {
				_data_->_tmp369_ = _data_->_p_it;
				_data_->_tmp370_ = gee_iterator_next (_data_->_tmp369_);
				if (!_data_->_tmp370_) {
					break;
				}
				_data_->_tmp371_ = _data_->_p_it;
				_data_->_tmp372_ = gee_iterator_get (_data_->_tmp371_);
				_data_->p = (TrfPersona*) _data_->_tmp372_;
				_data_->_tmp373_ = _data_->p;
				_data_->_tmp374_ = _g_object_ref0 (_data_->_tmp373_);
				_g_object_unref0 (_data_->ret);
				_data_->ret = _data_->_tmp374_;
...
"Tracker.");
	...
	_data_->_tmp376_ = _data_->ret;
	if (_data_->_tmp376_ != NULL) {
		_data_->_tmp377_ = _data_->avatar;
		_data_->_tmp375_ = _data_->_tmp377_ != NULL;
	} else {
		_data_->_tmp375_ = FALSE;
	}
	if (_data_->_tmp375_) {
		_data_->_tmp378_ = _data_->ret;
		_data_->_tmp379_ = _data_->avatar;
		_data_->_state_ = 5;
		_trf_persona_store_set_avatar (_data_->self, (FolksPersona*) _data_->_tmp378_, _data_->_tmp379_, trf_persona_store_add_persona_from_details_ready, _data_);
		return FALSE;
		_state_5:
