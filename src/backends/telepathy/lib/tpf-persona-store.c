/* tpf-persona-store.c generated by valac 0.20.1.30-1b1d, the Vala compiler
...
struct _TpfPersonaStoreNotifyConnectionCbAsyncData {
	...
	gint flags;
	TpConnectionAliasFlags _tmp28_;
	TpConnection* _tmp29_;
	TpConnectionAliasFlags _tmp30_;
	...
	TpContactMetadataStorageType _tmp38_;
	gchar** _tmp39_;
	...
	gchar* _tmp45_;
	FolksMaybeBool _tmp46_;
	TpConnection* _tmp47_;
	TpConnection* _tmp48_;
	const gchar* _tmp49_;
	const gchar* _tmp50_;
	GError * _inner_error_;
};
...
struct _TpfPersonaStorePopulateCountersData {
	...
	GPtrArray* _tmp62_;
	ZeitgeistTimeRange* _tmp63_;
	ZeitgeistTimeRange* _tmp64_;
	GPtrArray* _tmp65_;
	ZeitgeistMonitor* _tmp66_;
	ZeitgeistMonitor* _tmp67_;
	ZeitgeistLog* _tmp68_;
	ZeitgeistMonitor* _tmp69_;
	GError * _inner_error_;
};
...
static void _tpf_persona_store_debug_print_status (TpfPersonaStore* self, FolksDebug* debug) {
	...
	folks_debug_indent (_tmp93_);
	{
		...
		while (TRUE) {
			...
			_tmp105_ = g_strdup ((const gchar*) _tmp104_);
			id = _tmp105_;
			_tmp106_ = debug;
			_tmp107_ = id;
			folks_debug_print_line (_tmp106_, domain, level, "%s", _tmp107_);
			_g_free0 (id);
		}
		_g_object_unref0 (_id_list);
	}
	_tmp108_ = debug;
	folks_debug_unindent (_tmp108_);
	_tmp109_ = debug;
	_tmp110_ = self->priv->_avatars;
	_tmp111_ = gee_abstract_map_get_size ((GeeMap*) _tmp110_);
	_tmp112_ = _tmp111_;
	folks_debug_print_line (_tmp109_, domain, level, "Cached avatars for %u personas:", (guint) _tmp112_);
	_tmp113_ = debug;
	folks_debug_indent (_tmp113_);
	{
		...
		_tmp114_ = self->priv->_avatars;
		_tmp115_ = gee_abstract_map_get_keys ((GeeMap*) _tmp114_);
		_tmp116_ = _tmp115_;
		_tmp117_ = _tmp116_;
		...
		while (TRUE) {
			...
			_tmp120_ = _id_it;
			_tmp121_ = gee_iterator_next (_tmp120_);
			if (!_tmp121_) {
				break;
			}
			_tmp122_ = _id_it;
			...
			_tmp124_ = debug;
			_tmp125_ = id;
			folks_debug_print_line (_tmp124_, domain, level, "%s", _tmp125_);
			_g_free0 (id);
		}
		_g_object_unref0 (_id_it);
	}
	_tmp126_ = debug;
	folks_debug_unindent (_tmp126_);
	_tmp127_ = debug;
	folks_debug_print_line (_tmp127_, domain, level, "");
...
static void _tpf_persona_store_reset (TpfPersonaStore* self) {
	...
	_tmp1_ = _tmp0_;
	g_debug ("tpf-persona-store.vala:397: Resetting Tpf.PersonaStore %p ('%s')", self, _tmp1_);
	...
	_tmp10_ = gee_hash_map_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, TPF_TYPE_PERSONA, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	...
	_tmp14_ = gee_hash_set_new (TPF_TYPE_PERSONA, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL, NULL, NULL, NULL);
	...
	_tmp33_ = gee_hash_map_new (tp_contact_get_type (), NULL, NULL, TPF_TYPE_PERSONA, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	_g_object_unref0 (self->priv->_contact_persona_map);
	self->priv->_contact_persona_map = _tmp33_;
	_tmp34_ = folks_small_set_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, NULL, NULL, NULL, NULL, NULL, NULL);
...
static void _tpf_persona_store_remove_store (TpfPersonaStore* self, GeeSet* old_personas) {
	...
	if (_tmp0_ == TRUE) {
		...
		_tmp2_ = _tmp1_;
		g_debug ("tpf-persona-store.vala:447: Delaying removing store %s (%p) due to pen" \
"ding disconnect.", _tmp2_, self);
		...
		_tmp4_ = _tmp3_;
		g_debug ("tpf-persona-store.vala:453: Removing store %s (%p)", _tmp4_, self);
...
static void ___lambda6_ (TpfPersonaStore* self, TpAccount* a) {
	...
	if (_tmp1_ == _tmp2_) {
		...
		_tmp6_ = _tmp5_;
		g_debug ("tpf-persona-store.vala:513: Account %p (‘%s’) removed.", _tmp3_, _tmp6_);
...
static void ___lambda7_ (TpfPersonaStore* self, TpAccount* a, gboolean valid) {
	...
	if (_tmp5_) {
		...
		_tmp9_ = _tmp8_;
		g_debug ("tpf-persona-store.vala:522: Account %p (‘%s’) invalid.", _tmp6_, _tmp9_);
		_tmp10_ = self->priv->_persona_set;
		_tpf_persona_store_remove_store (self, (GeeSet*) _tmp10_);
	}
}
...
static void ___lambda8_ (TpfPersonaStore* self, TpAccount* a) {
	...
	if (_tmp1_ == _tmp2_) {
		...
		_tmp5_ = tp_account_get_display_name (_tmp4_);
		_tmp6_ = _tmp5_;
		g_debug ("tpf-persona-store.vala:531: Account %p (‘%s’) disabled.", _tmp3_, _tmp6_);
		_tmp7_ = self->priv->_persona_set;
		_tpf_persona_store_remove_store (self, (GeeSet*) _tmp7_);
	}
}
...
static void ___lambda9_ (TpfPersonaStore* self, GObject* o, GAsyncResult* r) {
	...
	__catch7_g_error:
	{
		...
		g_debug ("tpf-persona-store.vala:559: Failed to initialise favourite contacts: %" \
"s", _tmp4_);
...
static void _tpf_persona_store_logger_invalidated_cb (TpfPersonaStore* self) {
	...
	g_debug ("tpf-persona-store.vala:618: Lost connection to the telepathy-logger se" \
"rvice.");
	_g_object_unref0 (self->priv->_logger);
	self->priv->_logger = NULL;
}
...
static void ____lambda10_ (Block1Data* _data1_, GObject* o, GAsyncResult* r) {
	...
	if (_tmp1_ == FALSE) {
		...
		_tpf_persona_store_load_cache (self, _tmp2_, ____lambda11__gasync_ready_callback, g_object_ref (self));
	} else {
		gboolean _tmp3_;
		GeeHashSet* _tmp4_;
...
static void _tpf_persona_store_notify_connection_cb (TpfPersonaStore* self, GObject* s, GParamSpec* p) {
	...
	_tmp6_ = _tmp5_;
	g_debug ("tpf-persona-store.vala:680: Account '%s' connection changed to %p", _tmp3_, _tmp6_);
	_tmp7_ = account;
...
static gboolean _tpf_persona_store_notify_connection_cb_async_co (TpfPersonaStoreNotifyConnectionCbAsyncData* _data_) {
	...
	_data_->_tmp0_ = folks_persona_store_get_id ((FolksPersonaStore*) _data_->self);
	_data_->_tmp1_ = _data_->_tmp0_;
	g_debug ("tpf-persona-store.vala:752: _notify_connection_cb_async() for Tpf.Pers" \
"onaStore %p ('%s').", _data_->self, _data_->_tmp1_);
	...
	if (!_data_->_tmp19_) {
		g_debug ("tpf-persona-store.vala:770: %s", "Connection does not implement ContactList iface; " "legacy CMs are not supported any more.");
		_data_->_tmp20_ = _data_->self->priv->_persona_set;
		_tpf_persona_store_remove_store (_data_->self, (GeeSet*) _data_->_tmp20_);
		if (_data_->_state_ == 0) {
			g_simple_async_result_complete_in_idle (_data_->_async_result);
		} else {
			g_simple_async_result_complete (_data_->_async_result);
	...
	_data_->_tmp21_ = tpf_persona_store_get_account (_data_->self);
	_data_->_tmp22_ = _data_->_tmp21_;
	_data_->_tmp23_ = tp_account_get_connection (_data_->_tmp22_);
	_data_->_tmp24_ = _data_->_tmp23_;
	_data_->_tmp25_ = _g_object_ref0 (_data_->_tmp24_);
	_g_object_unref0 (_data_->self->priv->_conn);
	_data_->self->priv->_conn = _data_->_tmp25_;
	_data_->_tmp26_ = _data_->self->priv->_conn;
	g_signal_connect_object ((GObject*) _data_->_tmp26_, "notify::self-contact", (GCallback) __tpf_persona_store_self_contact_changed_cb_g_object_notify, _data_->self, 0);
	_data_->_tmp27_ = _data_->self->priv->_conn;
	g_signal_connect_object ((GObject*) _data_->_tmp27_, "notify::contact-list-state", (GCallback) __tpf_persona_store_contact_list_state_changed_cb_g_object_notify, _data_->self, 0);
	_data_->flags = 0;
	{
		_data_->_tmp29_ = _data_->self->priv->_conn;
		_data_->_state_ = 2;
		folks_tp_lowlevel_connection_get_alias_flags_async (_data_->_tmp29_, _tpf_persona_store_notify_connection_cb_async_ready, _data_);
		return FALSE;
		_state_2:
		_data_->_tmp30_ = 0;
		_data_->_tmp30_ = folks_tp_lowlevel_connection_get_alias_flags_finish (_data_->_res_, &_data_->_inner_error_);
		_data_->_tmp28_ = _data_->_tmp30_;
		if (_data_->_inner_error_ != NULL) {
			goto __catch8_g_error;
		}
		_data_->flags = (gint) _data_->_tmp28_;
	...
	__catch8_g_error:
	{
		...
		_data_->_tmp32_ = NULL;
		_data_->_tmp32_ = _ ("Failed to determine whether we can set aliases on Telepathy account '%" \
"s': %s");
		_data_->_tmp33_ = folks_persona_store_get_display_name ((FolksPersonaStore*) _data_->self);
		_data_->_tmp34_ = _data_->_tmp33_;
		_data_->_tmp35_ = _data_->e;
		_data_->_tmp36_ = _data_->_tmp35_->message;
		g_warning (_data_->_tmp32_, _data_->_tmp34_, _data_->_tmp36_);
	...
	_data_->_tmp38_ = 0;
	_data_->_tmp38_ = tp_connection_get_group_storage (_data_->_tmp37_);
	if (_data_->_tmp38_ != TP_CONTACT_METADATA_STORAGE_TYPE_NONE) {
		_data_->self->priv->_can_group_personas = FOLKS_MAYBE_BOOL_TRUE;
		_data_->_tmp39_ = _data_->self->priv->_always_writeable_properties;
		_data_->_tmp39__length1 = _data_->self->priv->_always_writeable_properties_length1;
		_data_->_tmp40_ = g_strdup ("groups");
		_vala_array_add7 (&_data_->self->priv->_always_writeable_properties, &_data_->self->priv->_always_writeable_properties_length1, &_data_->self->priv->__always_writeable_properties_size_, _data_->_tmp40_);
		g_object_notify ((GObject*) _data_->self, "always-writeable-properties");
	} else {
	...
	_data_->_tmp42_ = tp_connection_get_can_change_contact_list (_data_->_tmp41_);
	if (_data_->_tmp42_) {
	...
	_data_->_tmp43_ = _data_->flags;
	if ((_data_->_tmp43_ & TP_CONNECTION_ALIAS_FLAG_USER_SET) > 0) {
		_data_->new_can_alias = FOLKS_MAYBE_BOOL_TRUE;
		_data_->_tmp44_ = _data_->self->priv->_always_writeable_properties;
		_data_->_tmp44__length1 = _data_->self->priv->_always_writeable_properties_length1;
		_data_->_tmp45_ = g_strdup ("alias");
		_vala_array_add8 (&_data_->self->priv->_always_writeable_properties, &_data_->self->priv->_always_writeable_properties_length1, &_data_->self->priv->__always_writeable_properties_size_, _data_->_tmp45_);
		g_object_notify ((GObject*) _data_->self, "always-writeable-properties");
	}
	_data_->_tmp46_ = _data_->new_can_alias;
	_data_->self->priv->_can_alias_personas = _data_->_tmp46_;
	g_object_notify ((GObject*) _data_->self, "can-alias-personas");
	g_object_thaw_notify ((GObject*) _data_->self);
	_data_->_tmp47_ = _data_->self->priv->_conn;
	_tpf_persona_store_self_contact_changed_cb (_data_->self, (GObject*) _data_->_tmp47_, NULL);
	_data_->_tmp48_ = _data_->self->priv->_conn;
	_tpf_persona_store_contact_list_state_changed_cb (_data_->self, (GObject*) _data_->_tmp48_, NULL);
	_data_->_tmp49_ = folks_persona_store_get_id ((FolksPersonaStore*) _data_->self);
	_data_->_tmp50_ = _data_->_tmp49_;
	folks_internal_profiling_end ("notify connection for Tpf.PersonaStore " "(ID: %s)", _data_->_tmp50_, NULL);
	if (_data_->_state_ == 0) {
		g_simple_async_result_complete_in_idle (_data_->_async_result);
	} else {
		g_simple_async_result_complete (_data_->_async_result);
...
static gboolean _tpf_persona_store_load_cache_co (TpfPersonaStoreLoadCacheData* _data_) {
	...
	g_debug ("tpf-persona-store.vala:919: Loading cache for Tpf.PersonaStore %p ('%s" \
"').", _data_->self, _data_->_tmp25_);
	...
	if (_data_->_tmp27_ != NULL) {
		...
		g_debug ("tpf-persona-store.vala:925:     Cancelling ongoing loading operation (" \
"cancellable: %p).", _data_->_tmp28_);
	...
	if (_data_->_tmp36_ == TRUE) {
		...
		g_debug ("tpf-persona-store.vala:939:     Cancelled (cancellable: %p).", _data_->_tmp37_);
	...
	_data_->_tmp38_ = gee_hash_set_new (TPF_TYPE_PERSONA, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL, NULL, NULL, NULL);
...
static gboolean tpf_persona_store_real_flush_co (TpfPersonaStoreFlushData* _data_) {
	...
	_data_->_tmp0_ = folks_persona_store_get_id ((FolksPersonaStore*) _data_->self);
	_data_->_tmp1_ = _data_->_tmp0_;
	g_debug ("tpf-persona-store.vala:977: Flushing Tpf.PersonaStore %p (‘%s’).", _data_->self, _data_->_tmp1_);
...
static gboolean _tpf_persona_store_store_cache_co (TpfPersonaStoreStoreCacheData* _data_) {
	...
	g_debug ("tpf-persona-store.vala:1019: Storing cache for Tpf.PersonaStore %p ('%" \
"s').", _data_->self, _data_->_tmp16_);
...
static void _tpf_persona_store_unload_cache (TpfPersonaStore* self) {
	...
	_tmp0_ = folks_persona_store_get_id ((FolksPersonaStore*) self);
	_tmp1_ = _tmp0_;
	g_debug ("tpf-persona-store.vala:1031: Unloading cache for Tpf.PersonaStore %p (" \
"'%s').", self, _tmp1_);
	...
	if (_tmp2_ != NULL) {
		...
		g_debug ("tpf-persona-store.vala:1036:     Cancelling ongoing loading operation " \
...
static gboolean _tpf_persona_store_add_persona (TpfPersonaStore* self, TpfPersona* p) {
	...
	if (_tmp2_) {
		...
		_tmp6_ = _tmp5_;
		g_debug ("tpf-persona-store.vala:1068: Add persona %p with uid %s", _tmp3_, _tmp6_);
...
static gboolean _tpf_persona_store_remove_persona (TpfPersonaStore* self, TpfPersona* p) {
	...
	if (_tmp2_) {
		...
		_tmp5_ = folks_persona_get_uid ((FolksPersona*) _tmp4_);
		_tmp6_ = _tmp5_;
		g_debug ("tpf-persona-store.vala:1080: Remove persona %p with uid %s", _tmp3_, _tmp6_);
		_tmp7_ = self->priv->_personas;
		_tmp8_ = p;
		_tmp9_ = folks_persona_get_iid ((FolksPersona*) _tmp8_);
		_tmp10_ = _tmp9_;
...
static void _tpf_persona_store_contact_weak_notify_cb (TpfPersonaStore* self, GObject* obj) {
	...
	g_debug ("tpf-persona-store.vala:1101: Weak notify for TpContact %s", _tmp4_);
	persona = NULL;
	...
	if (_tmp10_) {
		...
		g_warning ("tpf-persona-store.vala:1115: A TpContact part of the ContactList is di" \
"sposed");
		_tmp11_ = folks_small_set_new (TPF_TYPE_PERSONA, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL, NULL, NULL, NULL);
		personas = _tmp11_;
		_tmp12_ = personas;
		_tmp13_ = persona;
...
TpfPersona* _tpf_persona_store_ensure_persona_for_contact (TpfPersonaStore* self, TpContact* contact) {
	...
	_tmp26_ = _tmp18_;
	g_debug ("tpf-persona-store.vala:1138: Persona %p with uid %s created for TpCont" \
...
static void _tpf_persona_store_self_contact_changed_cb (TpfPersonaStore* self, GObject* s, GParamSpec* p) {
	...
	if (_tmp14_) {
		...
		_tmp15_ = self_persona;
		_tmp16_ = _tpf_persona_store_remove_persona (self, _tmp15_);
		_tmp8_ = _tmp16_;
	} else {
		_tmp8_ = FALSE;
	}
	...
	if (_tmp17_) {
		...
		TpfPersona* _tmp19_;
		_tmp18_ = personas_removed;
		_tmp19_ = self_persona;
	...
	if (_tmp20_ != NULL) {
		...
		_tmp21_ = contact;
		_tmp22_ = _tpf_persona_store_ensure_persona_for_contact (self, _tmp21_);
		_g_object_unref0 (self->priv->_self_persona);
		self->priv->_self_persona = _tmp22_;
		_tmp23_ = self->priv->_self_persona;
		_tmp24_ = _tpf_persona_store_add_persona (self, _tmp23_);
		if (_tmp24_) {
	...
	_tmp27_ = personas_added;
	_tmp28_ = personas_removed;
	_folks_persona_store_emit_personas_changed ((FolksPersonaStore*) self, (GeeSet*) _tmp27_, (GeeSet*) _tmp28_, NULL, NULL, FOLKS_GROUP_DETAILS_CHANGE_REASON_NONE);
	self->priv->_got_initial_self_contact = TRUE;
	_tpf_persona_store_notify_if_is_quiescent (self);
...
static void _tpf_persona_store_contact_list_changed_cb (TpfPersonaStore* self, GPtrArray* added, GPtrArray* removed) {
	...
	_tmp7_ = _tmp6_;
	g_debug ("tpf-persona-store.vala:1203: contact list changed: %d added, %d remove" \
"d", _tmp4_, _tmp7_);
	_tmp8_ = added;
	_tmp9_ = _tmp8_->pdata;
		...
		for (contact_it = 0; contact_it < _tmp22__length1; contact_it = contact_it + 1) {
			...
			contact = _tmp23_;
			{
				...
				if (_tmp27_ == NULL) {
					...
					g_warning ("tpf-persona-store.vala:1227: Unknown TpContact removed from ContactLis" \
"t: %s", _tmp29_);
					_g_object_unref0 (persona);
					_g_object_unref0 (contact);
					continue;
...
static gboolean tpf_persona_store_change_alias_co (TpfPersonaStoreChangeAliasData* _data_) {
	...
	}
	{
		...
		g_debug ("tpf-persona-store.vala:1424: Changing alias of persona %s to '%s'.", _data_->_tmp10_, _data_->_tmp11_);
...
static GList* _tpf_persona_store_contact_info_set_to_list (FolksSmallSet* info_set) {
	...
	info_list = NULL;
	{
		...
		while (TRUE) {
			...
			const gchar* _tmp13_;
			TpContactInfoField* _tmp14_;
			gchar** _tmp15_;
			gint _tmp15__length1;
			TpContactInfoField* _tmp16_;
			gchar** _tmp17_;
			gint _tmp17__length1;
			TpContactInfoField* _tmp18_;
...
static void _tpf_persona_store_add_store_to_map (TpfPersonaStore* store) {
	...
	_tmp3_ = _tmp2_;
	g_debug ("tpf-persona-store.vala:1613: Adding PersonaStore %p ('%s') to map.", _tmp0_, _tmp3_);
	{
		GeeHashMap* _tmp4_;
...
static void _tpf_persona_store_remove_store_from_map (TpfPersonaStore* store) {
	...
	_tmp2_ = folks_persona_store_get_id ((FolksPersonaStore*) _tmp1_);
	_tmp3_ = _tmp2_;
	g_debug ("tpf-persona-store.vala:1638: Removing PersonaStore %p ('%s') from map.", _tmp0_, _tmp3_);
	{
		GeeHashMap* _tmp4_;
		_tmp4_ = tpf_persona_store__persona_stores_by_account;
		g_static_rec_mutex_lock (&__lock_tpf_persona_store__persona_stores_by_account);
...
TpfPersonaStore* tpf_persona_store_dup_for_account (TpAccount* account) {
	...
	store = NULL;
	_tmp0_ = account;
	g_debug ("tpf-persona-store.vala:1680: Tpf.PersonaStore.dup_for_account (%p):", _tmp0_);
	{
		GeeHashMap* _tmp1_;
		_tmp1_ = tpf_persona_store__persona_stores_by_account;
		g_static_rec_mutex_lock (&__lock_tpf_persona_store__persona_stores_by_account);
		{
			...
			if (_tmp7_ == NULL) {
				...
				g_debug ("tpf-persona-store.vala:1696:     Creating new PersonaStore.");
				_tmp8_ = account;
				...
				_tmp12_ = folks_persona_store_get_id ((FolksPersonaStore*) _tmp11_);
				_tmp13_ = _tmp12_;
				g_debug ("tpf-persona-store.vala:1701:     Found existing PersonaStore %p ('%s')" \
".", _tmp10_, _tmp13_);
			}
		}
		__finally17:
...
static GPtrArray* _tpf_persona_store_get_zeitgeist_event_templates (TpfPersonaStore* self) {
	...
	_tmp4_ = g_ptr_array_new ();
	templates = _tmp4_;
	_tmp5_ = g_object_ref ((GObject*) ev1);
...
static gboolean _tpf_persona_store_populate_counters_co (TpfPersonaStorePopulateCountersData* _data_) {
	...
	}
	{
		...
		_data_->_tmp5_ = NULL;
		_data_->_tmp5_ = _tpf_persona_store_get_zeitgeist_event_templates (_data_->self);
		...
		_data_->events = NULL;
		_data_->_state_ = 1;
		zeitgeist_log_find_events (_data_->_tmp6_, _data_->_tmp10_, _data_->_tmp11_, ZEITGEIST_STORAGE_STATE_ANY, (guint32) 0, ZEITGEIST_RESULT_TYPE_MOST_RECENT_EVENTS, NULL, _tpf_persona_store_populate_counters_ready, _data_);
		return FALSE;
		_state_1:
		_data_->_tmp12_ = NULL;
		...
		{
			_data_->_tmp14_ = _data_->self->priv->_personas;
			_data_->_tmp15_ = gee_abstract_map_get_values ((GeeMap*) _data_->_tmp14_);
			_data_->_tmp16_ = _data_->_tmp15_;
			_data_->_tmp17_ = _data_->_tmp16_;
			_data_->_tmp18_ = NULL;
			...
			while (TRUE) {
				...
				g_object_freeze_notify ((GObject*) _data_->_tmp24_);
				_data_->_tmp25_ = _data_->persona;
				_tpf_persona_reset_interaction (_data_->_tmp25_);
				_g_object_unref0 (_data_->persona);
			}
		...
		}
		{
			...
			_data_->_tmp27_ = NULL;
			_data_->_tmp27_ = _vala_zeitgeist_result_set_iterator (_data_->_tmp26_);
			_data_->_e_it = _data_->_tmp27_;
			while (TRUE) {
				_data_->_tmp28_ = _data_->_e_it;
				_data_->_tmp29_ = NULL;
				_data_->_tmp29_ = _vala_zeitgeist_result_set_next_value (_data_->_tmp28_);
				_data_->e = _data_->_tmp29_;
				...
				_data_->_tmp32_ = NULL;
				_data_->_tmp32_ = zeitgeist_event_get_subject (_data_->_tmp31_, 0);
				_data_->_tmp33_ = NULL;
				_data_->_tmp33_ = zeitgeist_subject_get_interpretation (_data_->_tmp32_);
				_data_->_tmp34_ = g_strdup (_data_->_tmp33_);
				_data_->interaction_type = _data_->_tmp34_;
				{
					_data_->i = 1;
					{
						_data_->_tmp35_ = TRUE;
						while (TRUE) {
							...
							_data_->_tmp40_ = 0;
							_data_->_tmp40_ = zeitgeist_event_num_subjects (_data_->_tmp39_);
							if (!(_data_->_tmp38_ < _data_->_tmp40_)) {
								break;
							}
							_data_->_tmp41_ = _data_->e;
							_data_->_tmp42_ = _data_->i;
							_data_->_tmp43_ = NULL;
							_data_->_tmp43_ = zeitgeist_event_get_subject (_data_->_tmp41_, _data_->_tmp42_);
							_data_->_tmp44_ = NULL;
							_data_->_tmp44_ = zeitgeist_subject_get_uri (_data_->_tmp43_);
							_data_->_tmp45_ = NULL;
							_data_->_tmp45_ = _tpf_persona_store_get_iid_from_event_metadata (_data_->self, _data_->_tmp44_);
							_data_->id = _data_->_tmp45_;
							_data_->_tmp46_ = _data_->id;
							_data_->_tmp47_ = _data_->interaction_type;
							_data_->_tmp48_ = _data_->e;
							_tpf_persona_store_increase_persona_counter (_data_->self, _data_->_tmp46_, _data_->_tmp47_, _data_->_tmp48_);
							_g_free0 (_data_->id);
						}
					}
				}
		...
		{
			_data_->_tmp49_ = folks_persona_store_get_personas ((FolksPersonaStore*) _data_->self);
			_data_->_tmp50_ = _data_->_tmp49_;
			_data_->_tmp51_ = gee_map_get_values (_data_->_tmp50_);
			...
			_data_->_vala1__persona_it = _data_->_tmp55_;
			while (TRUE) {
				_data_->_tmp56_ = _data_->_vala1__persona_it;
				_data_->_tmp57_ = FALSE;
				_data_->_tmp57_ = gee_iterator_next (_data_->_tmp56_);
				if (!_data_->_tmp57_) {
					break;
				}
				_data_->_tmp58_ = _data_->_vala1__persona_it;
				_data_->_tmp59_ = NULL;
				_data_->_tmp59_ = gee_iterator_get (_data_->_tmp58_);
				_data_->_vala1_persona = (FolksPersona*) _data_->_tmp59_;
				_data_->_tmp60_ = _data_->_vala1_persona;
				g_object_thaw_notify ((GObject*) _data_->_tmp60_);
	...
	__catch19_g_error:
	{
		g_clear_error (&_data_->_inner_error_);
		_data_->_inner_error_ = NULL;
		g_debug ("tpf-persona-store.vala:1805: Failed to fetch events from Zeitgeist");
	}
	__finally19:
	if (_data_->_inner_error_ != NULL) {
		g_critical ("file %s: line %d: uncaught error: %s (%s, %d)", __FILE__, __LINE__, _data_->_inner_error_->message, g_quark_to_string (_data_->_inner_error_->domain), _data_->_inner_error_->code);
	...
	_data_->_tmp61_ = _data_->self->priv->_monitor;
	if (_data_->_tmp61_ == NULL) {
		_data_->_tmp62_ = NULL;
		_data_->_tmp62_ = _tpf_persona_store_get_zeitgeist_event_templates (_data_->self);
		_data_->monitor_events = _data_->_tmp62_;
		_data_->_tmp63_ = zeitgeist_time_range_new_from_now ();
		g_object_ref_sink (_data_->_tmp63_);
		_data_->_tmp64_ = _data_->_tmp63_;
		_data_->_tmp65_ = _data_->monitor_events;
		_data_->monitor_events = NULL;
		_data_->_tmp66_ = zeitgeist_monitor_new (_data_->_tmp64_, _data_->_tmp65_);
		_g_object_unref0 (_data_->self->priv->_monitor);
		_data_->self->priv->_monitor = _data_->_tmp66_;
		_g_object_unref0 (_data_->_tmp64_);
		_data_->_tmp67_ = _data_->self->priv->_monitor;
		g_signal_connect_object (_data_->_tmp67_, "events-inserted", (GCallback) __tpf_persona_store_handle_new_interaction_zeitgeist_monitor_events_inserted, _data_->self, 0);
		_data_->_tmp68_ = _data_->self->priv->_log;
		_data_->_tmp69_ = _data_->self->priv->_monitor;
		zeitgeist_log_install_monitor (_data_->_tmp68_, _data_->_tmp69_);
...
static GObject * tpf_persona_store_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	_tmp2_ = tpf_persona_store_get_account (self);
	_tmp3_ = _tmp2_;
	g_debug ("tpf-persona-store.vala:262: Creating new Tpf.PersonaStore %p ('%s') fo" \
...
static void tpf_persona_store_finalize (GObject* obj) {
	...
	_tmp0_ = folks_persona_store_get_id ((FolksPersonaStore*) self);
	_tmp1_ = _tmp0_;
	g_debug ("tpf-persona-store.vala:279: Destroying Tpf.PersonaStore %p ('%s').", self, _tmp1_);
	_tpf_persona_store_reset (self);
