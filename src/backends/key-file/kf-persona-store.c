/* kf-persona-store.c generated by valac 0.20.1.30-1b1d, the Vala compiler
...
struct _FolksBackendsKfPersonaStorePrepareData {
	...
	const gchar* _tmp54_;
	FolksBackendsKfPersona* _tmp55_;
	GeeHashMap* _tmp56_;
	FolksBackendsKfPersona* _tmp57_;
	const gchar* _tmp58_;
	const gchar* _tmp59_;
	FolksBackendsKfPersona* _tmp60_;
	GeeHashSet* _tmp61_;
	FolksBackendsKfPersona* _tmp62_;
	GeeHashMap* _tmp63_;
	gint _tmp64_;
	gint _tmp65_;
	GeeHashSet* _tmp66_;
...
struct _FolksBackendsKfPersonaStoreAddPersonaFromDetailsData {
	...
	FolksBackendsKfPersona* persona;
	const gchar* _tmp25_;
	FolksBackendsKfPersona* _tmp26_;
	GeeHashMap* _tmp27_;
	FolksBackendsKfPersona* _tmp28_;
	const gchar* _tmp29_;
	const gchar* _tmp30_;
	FolksBackendsKfPersona* _tmp31_;
	GeeMultiMap* _tmp32_;
	FolksBackendsKfPersona* _tmp33_;
	GeeMultiMap* _tmp34_;
	GeeMultiMap* _tmp35_;
	FolksBackendsKfPersona* _tmp36_;
	GeeMultiMap* _tmp37_;
	GError* e;
	GError* _tmp38_;
	const gchar* _tmp39_;
	...
	FolksBackendsKfPersona* _tmp42_;
	GError * _inner_error_;
};
...
static const gchar* FOLKS_BACKENDS_KF_PERSONA_STORE__always_writeable_properties[4] = {"alias", "im-addresses", "web-service-addresses", "anti-links"};
...
static gboolean folks_backends_kf_persona_store_real_prepare_co (FolksBackendsKfPersonaStorePrepareData* _data_) {
	...
	}
	{
		...
		_data_->filename = _data_->_tmp7_;
		_data_->_tmp8_ = g_key_file_new ();
		_g_key_file_free0 (_data_->self->priv->_key_file);
		_data_->self->priv->_key_file = _data_->_tmp8_;
		while (TRUE) {
			{
				...
				_data_->_tmp10_ = NULL;
				memset (&_data_->_tmp11_, 0, sizeof (gsize));
				_data_->_state_ = 1;
				g_file_load_contents_async (_data_->_tmp9_, NULL, folks_backends_kf_persona_store_prepare_ready, _data_);
				return FALSE;
				_state_1:
				g_file_load_contents_finish (_data_->_tmp9_, _data_->_res_, &_data_->_tmp10_, &_data_->_tmp11_, NULL, &_data_->_inner_error_);
				_data_->contents = (g_free (_data_->contents), NULL);
				_data_->contents = _data_->_tmp10_;
				_data_->contents_length1 = _data_->_tmp11_;
				_data_->_contents_size_ = _data_->contents_length1;
				if (_data_->_inner_error_ != NULL) {
					_data_->contents = (g_free (_data_->contents), NULL);
					goto __catch8_g_error;
				}
				_data_->_tmp12_ = _data_->contents;
				_data_->_tmp12__length1 = _data_->contents_length1;
				_data_->contents_s = (const gchar*) _data_->_tmp12_;
				_data_->_tmp13_ = folks_persona_store_get_id ((FolksPersonaStore*) _data_->self);
				_data_->_tmp14_ = _data_->_tmp13_;
				folks_internal_profiling_point ("loaded file in " "Kf.PersonaStore (ID: %s)", _data_->_tmp14_, NULL);
				_data_->_tmp15_ = _data_->contents_s;
				_data_->_tmp16_ = strlen (_data_->_tmp15_);
				_data_->_tmp17_ = _data_->_tmp16_;
				if (_data_->_tmp17_ > 0) {
					_data_->_tmp18_ = _data_->self->priv->_key_file;
					_data_->_tmp19_ = _data_->contents_s;
					_data_->_tmp20_ = _data_->contents_s;
					_data_->_tmp21_ = strlen (_data_->_tmp20_);
					_data_->_tmp22_ = _data_->_tmp21_;
					g_key_file_load_from_data (_data_->_tmp18_, _data_->_tmp19_, (gsize) _data_->_tmp22_, G_KEY_FILE_KEEP_COMMENTS, &_data_->_inner_error_);
					if (_data_->_inner_error_ != NULL) {
						_data_->contents = (g_free (_data_->contents), NULL);
						goto __catch8_g_error;
					}
					_data_->_tmp23_ = folks_persona_store_get_id ((FolksPersonaStore*) _data_->self);
					_data_->_tmp24_ = _data_->_tmp23_;
					folks_internal_profiling_point ("parsed data in " "Kf.PersonaStore (ID: %s)", _data_->_tmp24_, NULL);
				}
				_data_->contents = (g_free (_data_->contents), NULL);
				break;
			}
			...
			__catch8_g_error:
			{
				...
				if (!g_error_matches (_data_->_tmp25_, G_IO_ERROR, G_IO_ERROR_NOT_FOUND)) {
					_data_->_tmp26_ = NULL;
					_data_->_tmp26_ = _ ("The relationship key file '%s' could not be loaded: %s");
					_data_->_tmp27_ = _data_->filename;
					_data_->_tmp28_ = _data_->e1;
					_data_->_tmp29_ = _data_->_tmp28_->message;
					g_warning (_data_->_tmp26_, _data_->_tmp27_, _data_->_tmp29_);
			...
			_data_->_tmp31_ = NULL;
			_data_->_tmp31_ = g_file_get_parent (_data_->_tmp30_);
			_data_->parent_dir = _data_->_tmp31_;
			{
				_data_->_tmp32_ = _data_->parent_dir;
				g_file_make_directory_with_parents (_data_->_tmp32_, NULL, &_data_->_inner_error_);
				if (_data_->_inner_error_ != NULL) {
			...
			__catch9_g_error:
			{
				...
				_data_->_inner_error_ = NULL;
				_data_->_tmp33_ = _data_->e3;
				if (!g_error_matches (_data_->_tmp33_, G_IO_ERROR, G_IO_ERROR_EXISTS)) {
					_data_->_tmp34_ = NULL;
					_data_->_tmp34_ = _ ("The relationship key file directory '%s' could not be created: %s");
					_data_->_tmp35_ = _data_->parent_dir;
					_data_->_tmp36_ = NULL;
					_data_->_tmp36_ = g_file_get_path (_data_->_tmp35_);
					_data_->_tmp37_ = _data_->_tmp36_;
					_data_->_tmp38_ = _data_->e3;
					_data_->_tmp39_ = _data_->_tmp38_->message;
					g_warning (_data_->_tmp34_, _data_->_tmp37_, _data_->_tmp39_);
			...
			{
				_data_->_tmp40_ = _data_->self->priv->_file;
				_data_->_state_ = 2;
				g_file_create_async (_data_->_tmp40_, G_FILE_CREATE_PRIVATE, G_PRIORITY_DEFAULT, NULL, folks_backends_kf_persona_store_prepare_ready, _data_);
				return FALSE;
				_state_2:
				_data_->_tmp41_ = NULL;
				_data_->_tmp41_ = g_file_create_finish (_data_->_tmp40_, _data_->_res_, &_data_->_inner_error_);
				_data_->stream = _data_->_tmp41_;
				if (_data_->_inner_error_ != NULL) {
					goto __catch10_g_error;
				}
				_data_->_tmp42_ = _data_->stream;
				_data_->_state_ = 3;
				g_output_stream_close_async ((GOutputStream*) _data_->_tmp42_, G_PRIORITY_DEFAULT, NULL, folks_backends_kf_persona_store_prepare_ready, _data_);
				return FALSE;
				_state_3:
			...
			__catch10_g_error:
			{
				...
				if (!g_error_matches (_data_->_tmp43_, G_IO_ERROR, G_IO_ERROR_EXISTS)) {
					_data_->_tmp44_ = NULL;
					_data_->_tmp44_ = _ ("The relationship key file '%s' could not be created: %s");
					_data_->_tmp45_ = _data_->filename;
					_data_->_tmp46_ = _data_->e2;
					_data_->_tmp47_ = _data_->_tmp46_->message;
					g_warning (_data_->_tmp44_, _data_->_tmp45_, _data_->_tmp47_);
					g_signal_emit_by_name ((FolksPersonaStore*) _data_->self, "removed");
		...
		_data_->_tmp48_ = _data_->self->priv->_key_file;
		memset (&_data_->_tmp49_, 0, sizeof (gsize));
		_data_->_tmp50_ = NULL;
		_data_->_tmp50_ = g_key_file_get_groups (_data_->_tmp48_, &_data_->_tmp49_);
		_data_->groups = _data_->_tmp50_;
		_data_->groups_length1 = _data_->_tmp49_;
		_data_->_groups_size_ = _data_->groups_length1;
		_data_->_tmp51_ = gee_hash_set_new (FOLKS_BACKENDS_KF_TYPE_PERSONA, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL, NULL, NULL, NULL);
		_data_->added_personas = _data_->_tmp51_;
		_data_->_tmp52_ = _data_->groups;
		_data_->_tmp52__length1 = _data_->groups_length1;
		{
			_data_->persona_id_collection_length1 = 0;
			_data_->_persona_id_collection_size_ = 0;
			_data_->persona_id_collection = _data_->_tmp52_;
			_data_->persona_id_collection_length1 = _data_->_tmp52__length1;
			for (_data_->persona_id_it = 0; _data_->persona_id_it < _data_->_tmp52__length1; _data_->persona_id_it = _data_->persona_id_it + 1) {
				_data_->_tmp53_ = g_strdup (_data_->persona_id_collection[_data_->persona_id_it]);
				_data_->persona_id = _data_->_tmp53_;
				{
					_data_->_tmp54_ = _data_->persona_id;
					_data_->_tmp55_ = folks_backends_kf_persona_new (_data_->_tmp54_, (FolksPersonaStore*) _data_->self);
					_data_->persona = _data_->_tmp55_;
					_data_->_tmp56_ = _data_->self->priv->_personas;
					_data_->_tmp57_ = _data_->persona;
					_data_->_tmp58_ = folks_persona_get_iid ((FolksPersona*) _data_->_tmp57_);
					_data_->_tmp59_ = _data_->_tmp58_;
					_data_->_tmp60_ = _data_->persona;
					gee_abstract_map_set ((GeeAbstractMap*) _data_->_tmp56_, _data_->_tmp59_, _data_->_tmp60_);
					_data_->_tmp61_ = _data_->added_personas;
		...
		_data_->_tmp63_ = _data_->self->priv->_personas;
		_data_->_tmp64_ = gee_abstract_map_get_size ((GeeMap*) _data_->_tmp63_);
		_data_->_tmp65_ = _data_->_tmp64_;
		if (_data_->_tmp65_ > 0) {
			_data_->_tmp66_ = _data_->added_personas;
			_folks_persona_store_emit_personas_changed ((FolksPersonaStore*) _data_->self, (GeeSet*) _data_->_tmp66_, NULL, NULL, NULL, FOLKS_GROUP_DETAILS_CHANGE_REASON_NONE);
		}
	...
	_data_->_tmp67_ = folks_persona_store_get_id ((FolksPersonaStore*) _data_->self);
	_data_->_tmp68_ = _data_->_tmp67_;
	folks_internal_profiling_end ("preparing Kf.PersonaStore (ID: %s)", _data_->_tmp68_, NULL);
	if (_data_->_state_ == 0) {
		g_simple_async_result_complete_in_idle (_data_->_async_result);
	} else {
		g_simple_async_result_complete (_data_->_async_result);
...
static gboolean folks_backends_kf_persona_store_real_remove_persona_co (FolksBackendsKfPersonaStoreRemovePersonaData* _data_) {
	...
	g_debug ("kf-persona-store.vala:340: Removing Persona '%s' (IID '%s', group '%s'" \
...
static gboolean folks_backends_kf_persona_store_real_add_persona_from_details_co (FolksBackendsKfPersonaStoreAddPersonaFromDetailsData* _data_) {
	...
	_data_->im_addresses = _data_->_tmp8_;
	_data_->_tmp9_ = _data_->details;
	_data_->_tmp10_ = NULL;
	_data_->_tmp10_ = folks_persona_store_detail_key (FOLKS_PERSONA_DETAIL_WEB_SERVICE_ADDRESSES);
	_data_->_tmp11_ = NULL;
	_data_->_tmp11_ = g_hash_table_lookup (_data_->_tmp9_, _data_->_tmp10_);
	_data_->val2 = (GValue*) _data_->_tmp11_;
	_data_->_tmp13_ = _data_->val2;
	if (_data_->_tmp13_ != NULL) {
		_data_->_tmp14_ = _data_->val2;
		_data_->_tmp15_ = NULL;
		_data_->_tmp15_ = g_value_get_object (_data_->_tmp14_);
		_data_->_tmp12_ = G_TYPE_CHECK_INSTANCE_CAST (_data_->_tmp15_, GEE_TYPE_MULTI_MAP, GeeMultiMap);
	} else {
		_data_->_tmp12_ = NULL;
	}
	_data_->_tmp16_ = _data_->_tmp12_;
	_data_->_tmp17_ = _g_object_ref0 (_data_->_tmp16_);
	_data_->web_service_addresses = _data_->_tmp17_;
	g_debug ("kf-persona-store.vala:388: Adding Persona from details.");
	_data_->persona_id = NULL;
	{
		...
		while (TRUE) {
			...
			if (!_data_->_tmp19_) {
				_data_->_tmp20_ = _data_->self->priv->_key_file;
				_data_->_tmp21_ = _data_->persona_id;
				_data_->_tmp22_ = FALSE;
				_data_->_tmp22_ = g_key_file_has_group (_data_->_tmp20_, _data_->_tmp21_);
			...
			_data_->_tmp23_ = 0U;
			_data_->_tmp23_ = g_random_int ();
			_data_->_tmp24_ = NULL;
			_data_->_tmp24_ = g_strdup_printf ("%u", _data_->_tmp23_);
			_g_free0 (_data_->persona_id);
			_data_->persona_id = _data_->_tmp24_;
		}
	}
	_data_->_tmp25_ = _data_->persona_id;
	_data_->_tmp26_ = folks_backends_kf_persona_new (_data_->_tmp25_, (FolksPersonaStore*) _data_->self);
	...
	gee_abstract_map_set ((GeeAbstractMap*) _data_->_tmp27_, _data_->_tmp30_, _data_->_tmp31_);
	{
		...
		if (_data_->_tmp32_ != NULL) {
			...
			_data_->_state_ = 1;
			folks_im_details_change_im_addresses ((FolksImDetails*) _data_->_tmp33_, _data_->_tmp34_, folks_backends_kf_persona_store_add_persona_from_details_ready, _data_);
			return FALSE;
			_state_1:
		...
		_data_->_tmp35_ = _data_->web_service_addresses;
		if (_data_->_tmp35_ != NULL) {
			_data_->_tmp36_ = _data_->persona;
			_data_->_tmp37_ = _data_->web_service_addresses;
			_data_->_state_ = 2;
			folks_web_service_details_change_web_service_addresses ((FolksWebServiceDetails*) _data_->_tmp36_, _data_->_tmp37_, folks_backends_kf_persona_store_add_persona_from_details_ready, _data_);
			return FALSE;
			_state_2:
			folks_web_service_details_change_web_service_addresses_finish ((FolksWebServiceDetails*) _data_->_tmp36_, _data_->_res_, &_data_->_inner_error_);
			if (_data_->_inner_error_ != NULL) {
				if (_data_->_inner_error_->domain == FOLKS_PROPERTY_ERROR) {
					goto __catch12_folks_property_error;
				}
	...
	__catch12_folks_property_error:
	{
		...
		_data_->_tmp39_ = _data_->_tmp38_->message;
		_data_->_tmp40_ = g_error_new_literal (FOLKS_PERSONA_STORE_ERROR, FOLKS_PERSONA_STORE_ERROR_CREATE_FAILED, _data_->_tmp39_);
	...
	_data_->_tmp41_ = folks_small_set_new (FOLKS_BACKENDS_KF_TYPE_PERSONA, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL, NULL, NULL, NULL);
	_data_->personas = _data_->_tmp41_;
	_data_->_tmp42_ = _data_->persona;
	gee_abstract_collection_add ((GeeAbstractCollection*) _data_->personas, _data_->_tmp42_);
...
static gboolean folks_backends_kf_persona_store_save_key_file_co (FolksBackendsKfPersonaStoreSaveKeyFileData* _data_) {
	...
	_data_->_tmp5_ = _data_->_tmp4_;
	g_debug ("kf-persona-store.vala:446: Saving key file '%s'.", _data_->_tmp5_);
